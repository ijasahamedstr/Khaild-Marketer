...
/* ---------------- COMPONENT ---------------- */
const Service3: React.FC<Props> = ({ onSubmit }) => {
  useEffect(() => {
   ...
   {/* DEVELOPER CHECKBOXES */}
      ...
      <Box sx={{ display: "grid", gap: 3 }}>
        {DROPDOWN_FIELDS.map((field, i) => (
          ...
          // 1. Wrapper Box for positioning (Relative)
          <Box key={i} sx={{ position: "relative" }}>
            
            {/* 2. GLOW EFFECT (Always Visible) */}
            <Box
              sx={{
                position: "absolute",
...
{/* DEVELOPER - HIDE IF مستأجر */}
     ...
     {/* AREA - HIDDEN IF مستأجر (isChecked2) OR IF APARTMENT */}
...
{/* ---------------- ADDITIONAL NOTES ---------------- */}
 ...
 {/* ---------------- CONTACT CHANNELS ---------------- */}
      ...
      <Box sx={{ mt: 6, textAlign: "center" }}>
        <Button variant="contained" color="primary" size="large" sx={{ fontFamily: TAJAWAL, fontWeight: 700, px: 8,background: GRADIENT, py: 1.5, fontSize: "1.3rem" }} onClick={handleSubmit}>
          إرسال الطلب
        </Button>
      </Box>
